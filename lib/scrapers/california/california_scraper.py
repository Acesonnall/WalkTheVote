...
async def get_election_offices():
    ...
    emails = [i.strip('E-Mail:').replace('\xa0', '').strip() for i in county_info if
              '@' in i]
    ...
    hours = [i.strip('Hours: ').strip('(707) 263-2742 FaxHours:').strip('\n\t') for i in
             county_info if 'Hours: ' in i]

    add_id = ['Street', 'Road', 'Ave', 'Court ', 'lane', 'Blvd', 'Hwy', 'Drive',
              'Place', 'St.', 'Real', 'Square', 'Memorial']
    ...
    replace_adds = ['Martinez, CA 94553', 'San Francisco, CA 94102-4635',
                    'Downieville, CA 95936-0398', 'Ventura, CA 93009-1200']
    ...
    with open(os.path.join(ROOT_DIR, r"scrapers\california\california.json"), 'w') as f:
